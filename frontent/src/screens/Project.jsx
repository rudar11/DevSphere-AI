      ...
      <section className="left relative flex flex-col h-full min-w-96 bg-slate-300">
        ...
        <header className='flex justify-between items-center p-2 px-4 w-full bg-slate-100'>
          ...
          <button
            ...
            <small>  <p>Add Collaborator</p></small>
          </button>
        ...
        <div className={`sidepanel w-full h-full flex flex-col gap-2 absolute bg-slate-50 transition-all  ${isSidePanelOpen ? 'translate-x-0' : '-translate-x-full'} top-0`}>
          ...
          <div className="users flex flex-col gap-2">
            ...
            {project.users && project.users.map(user => {
              ...
              return (
                <div key={userKey} className="user cursor-pointer hover:bg-slate-200  p-2 flex gap-2 items-center">
                  <div className='aspect-square rounded-full w-fit h-fit flex items-center justify-center p-5 text-white bg-slate-600'>
                    <i className='ri-user-fill absolute'></i>
                  </div>

                  <h1 className='font-semibold text-lg'>{typeof user === 'object' && user != null ? user.email : ''}</h1>
                </div>
              )
            })}
          </div>
